                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="sticky top-0 bg-card z-10 text-right font-bold">Net</TableHead>
                  ...
                  <TableBody>
                    {weeklyData.map((w, i) => (
                      <TableRow key={i} className={`${w.net < 0 ? 'bg-red-50/40' : i%2===0?'bg-white':'bg-muted/20'} ${w.isCurrentWeek ? 'ring-1 ring-inset ring-indigo-300' : ''}`}>
                        ...
                        <TableCell className={`text-right font-bold text-xs ${w.net>=0?'text-emerald-600':'text-red-600'}`}>
                    ...
                    return (
                      <TableRow key={i} className={m.inflow === 0 ? 'bg-amber-50/50' : i%2===0?'bg-white':'bg-muted/20'}>
                        ...
                        <TableCell className="text-right text-xs">
                          {inflowDelta !== null ? (
                            <span className={inflowDelta >= 0 ? 'text-emerald-600' : 'text-red-600'}>
                              {inflowDelta >= 0 ? '▲' : '▼'} ₹{Math.abs(inflowDelta).toLocaleString('en-IN')}
                            </span>
                        ...
                        <TableCell className="text-right text-xs">
                          {outflowDelta !== null ? (
                            <span className={outflowDelta <= 0 ? 'text-emerald-600' : 'text-red-600'}>
                              {outflowDelta <= 0 ? '▼' : '▲'} ₹{Math.abs(outflowDelta).toLocaleString('en-IN')}
                            </span>
                          ) : <span className="text-muted-foreground">—</span>}
                        </TableCell>
                        <TableCell className={`text-right font-semibold ${m.net>=0?'text-emerald-600':'text-red-600'}`}>
