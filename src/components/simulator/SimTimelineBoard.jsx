...
// FIXED: Now uses the same week boundary reference point as getFinancialWeekStartDate
function getFinancialWeekNumber(date) {
  ...
  // Get April 1st and adjust to the Monday of its week (matching getFinancialWeekStartDate logic)
  const aprilFirst = new Date(year, 3, 1);
  const dayOffset = aprilFirst.getDay();
  const weekStartRef = new Date(aprilFirst);
  weekStartRef.setDate(aprilFirst.getDate() - dayOffset);
  
  // Calculate week number from this adjusted reference point
  const daysFromWeekStart = Math.floor((d - weekStartRef) / 86400000);
  const weekNumber = Math.floor(daysFromWeekStart / 7) + 1;
...
export default function SimTimelineBoard({
  ...
  const onDragEnd = useCallback(({ source, destination, draggableId }) => {
    ...
    const newDateRaw = weekStart(dstWeek);
