...
export default function SimTable({ weeklyData, bankAccounts }) {
  ...
  const totals = weeklyData.reduce((acc, w) => ({
    ...
    fundingIn:   acc.fundingIn   + (w.fundingInflow || 0),
    repayOut:    acc.repayOut    + (w.repaymentOutflow || 0),
  }), { baseInflow: 0, simInflow: 0, baseOutflow: 0, simOutflow: 0, baseNet: 0, simNet: 0, delta: 0, fundingIn: 0, repayOut: 0 });
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {hasFunding && <TableHead className="text-right text-purple-700 text-xs">Funding In</TableHead>}
                {hasFunding && <TableHead className="text-right text-xs" style={{ color: '#b91c1c' }}>Repayment</TableHead>}
                    ...
                    <TableRow key={i} style={{ background: i % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.02)' }}
                      ...
                      <TableCell className="text-xs font-medium whitespace-nowrap">
                        <span className="flex items-center gap-1">
                          {w.label}{w.isCurrentWeek && <span className="text-indigo-400">•</span>}
                          {hasAdj && (
                            <Tooltip>
                              ...
                              <TooltipContent className="text-xs max-w-xs">
                                {w.simItems.map((s, j) => (
                                  <p key={j}>{s.split ? '⑂ ' : ''}{s.label} {s.type === 'inflow' ? '↑' : '↓'} {INR(s.amount)}{s.hypo ? ' (sim)' : ''}{s.funding ? ' 💰' : ''}</p>
                                ))}
                              </TooltipContent>
                            </Tooltip>
                          )}
                      ...
                      <TableCell className="text-right text-red-600 text-xs">
                        {w.simItems.some(s => s.type === 'outflow' && s.split) ? '⑂ ' : ''}
                        {INR(w.simOutflow)}
                      </TableCell>
                      ...
                      {hasFunding && <TableCell className="text-right text-xs text-purple-700 font-medium">{(w.fundingInflow||0) > 0 ? INR(w.fundingInflow) : '—'}</TableCell>}
                      {hasFunding && <TableCell className="text-right text-xs font-medium" style={{ color: (w.repaymentOutflow||0) > 0 ? '#b91c1c' : undefined }}>{(w.repaymentOutflow||0) > 0 ? INR(w.repaymentOutflow) : '—'}</TableCell>}
            ...
            <tfoot>
              <tr className="border-t bg-secondary font-semibold text-xs">
                ...
                {hasFunding && <td className="p-2 text-right text-purple-700">{INR(totals.fundingIn)}</td>}
                {hasFunding && <td className="p-2 text-right" style={{ color: '#b91c1c' }}>{INR(totals.repayOut)}</td>}
                <td className="p-2" />
              </tr>
            </tfoot>
          </Table>
